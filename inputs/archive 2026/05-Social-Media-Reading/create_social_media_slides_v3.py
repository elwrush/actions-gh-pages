"""
Create Social Media Society Slideshow v3 (Correct Thai Vocabulary)
Following validated outline with proper Thai script
"""
...
def create_content_slide_with_bold(slides_service, presentation_id, title, body_parts):
    """Create content slide with bold text support. body_parts is a list of (text, is_bold) tuples."""
    slide_id = _generate_id()
    header_id, title_id, body_id = _generate_id(), _generate_id(), _generate_id()
    
    # Build full text and track bold ranges
    full_text = ""
    bold_ranges = []
    ...
    # Add bold formatting for specific ranges
...
def create_content_slide(slides_service, presentation_id, title, body):
    """Simple content slide without bold formatting."""
    return create_content_slide_with_bold(slides_service, presentation_id, title, [(body, False)])

def main():
    print("Creating Social Media Society slideshow v3...")
    ...
    presentation = slides_service.presentations().create(body={'title': '06-01-26-Social-Media-Society-Slides-v3'}).execute()
    ...
    # Slide 3: Vocabulary 1 - antisocial (with bold)
    ...
    # Slide 4: Vocabulary 2 - hikikomori (with bold)
    ...
    # Slide 5: Vocabulary 3 - popularity contest (with bold)
    create_content_slide_with_bold(slides_service, presentation_id, "Vocabulary: popularity contest", [
        ...
        ("School should not be a ", False),
        ("popularity contest", True),
        (".\n\n", False),
        ("โรงเรียนไม่ควรเป็น", False),
