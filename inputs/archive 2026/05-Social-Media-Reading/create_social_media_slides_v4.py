"""
Create Social Media Society Slideshow v4
With transition slides and proper contextualizing vocabulary
"""
...
def create_transition_slide(slides_service, presentation_id, phrase):
    """Create a transition slide with centered text on maroon background."""
    slide_id = _generate_id()
    ...
    all_requests = [
        ...
        {'createShape': {'objectId': text_id, 'shapeType': 'TEXT_BOX', 'elementProperties': {'pageObjectId': slide_id, 'size': {'width': {'magnitude': inches(8), 'unit': 'EMU'}, 'height': {'magnitude': inches(2), 'unit': 'EMU'}}, 'transform': {'scaleX': 1, 'scaleY': 1, 'translateX': inches(1), 'translateY': inches(2), 'unit': 'EMU'}}}},
        {'insertText': {'objectId': text_id, 'text': phrase}},
        {'updateTextStyle': {'objectId': text_id, 'style': {'foregroundColor': {'opaqueColor': {'rgbColor': BRAND_COLORS['white']}}, 'fontSize': {'magnitude': 36, 'unit': 'PT'}, 'bold': True, 'fontFamily': 'Arial'}, 'textRange': {'type': 'ALL'}, 'fields': 'foregroundColor,fontSize,bold,fontFamily'}},
        {'updateParagraphStyle': {'objectId': text_id, 'style': {'alignment': 'CENTER'}, 'textRange': {'type': 'ALL'}, 'fields': 'alignment'}}
    ]
    
    slides_service.presentations().batchUpdate(presentationId=presentation_id, body={'requests': all_requests}).execute()
    print(f"Created: TRANSITION - {phrase[:30]}...")
    return slide_id

def create_content_slide_with_bold(slides_service, presentation_id, title, body_parts):
    """Create content slide with bold text support."""
    slide_id = _generate_id()
    header_id, title_id, body_id = _generate_id(), _generate_id(), _generate_id()
    
    full_text = ""
    bold_ranges = []
    for text, is_bold in body_parts:
        if is_bold:
            bold_ranges.append((len(full_text), len(full_text) + len(text)))
        full_text += text
    ...
    all_requests = [
        ...
        {'insertText': {'objectId': body_id, 'text': full_text}},
...
def create_content_slide(slides_service, presentation_id, title, body):
    return create_content_slide_with_bold(slides_service, presentation_id, title, [(body, False)])

def main():
    print("Creating Social Media Society slideshow v4...")
    ...
    presentation = slides_service.presentations().create(body={'title': '06-01-26-Social-Media-Society-Slides-v4'}).execute()
    ...
    create_transition_slide(slides_service, presentation_id, "Before we dive in...\nLet's arm ourselves with some key words")
    
    # Slide 4: Vocabulary 1 - antisocial
    create_content_slide_with_bold(slides_service, presentation_id, "Vocabulary: antisocial", [
        ("antisocial /ˌæntiˈsəʊʃəl/: ไม่ชอบสังคม\n\n", False),
        ("Some teenagers become ", False),
        ("antisocial", True),
        (" and prefer staying in their rooms.\n\n", False),
        ("วัยรุ่นบางคน", False),
        ("ไม่ชอบสังคม", True),
        ("และชอบอยู่ในห้องคนเดียว", False)
    ])
    
    # Slide 5: Vocabulary 2 - hikikomori
    create_content_slide_with_bold(slides_service, presentation_id, "Vocabulary: hikikomori", [
        ("hikikomori /ˌhɪkiˈkɒməri/: คนที่หลบหนีสังคม\n\n", False),
        ("Toshi had such extreme ", False),
        ("hikikomori", True),
        (" that he had not left his room for a week.\n\n", False),
        ("โทชิมีอาการ", False),
        ("ฮิกิโกะโมริ", True),
        ("อย่างรุนแรงจนไม่ได้ออกจากห้องมาหนึ่งสัปดาห์", False)
    ])
    
    # Slide 6: Vocabulary 3 - popularity contest
    create_content_slide_with_bold(slides_service, presentation_id, "Vocabulary: popularity contest", [
        ("popularity contest /ˌpɒpjuˈlærəti ˈkɒntest/: การแข่งขันเรื่องความนิยม\n\n", False),
        ("Instagram has become a ", False),
        ("popularity contest", True),
        (" where people compete for likes and followers.\n\n", False),
        ("อินสตาแกรมกลายเป็น", False),
        ("การแข่งขันเรื่องความนิยม", True),
        ("ที่คนแข่งกันหายอดไลค์และผู้ติดตาม", False)
    ])
    
    # Slide 7: TRANSITION - Challenge
    create_transition_slide(slides_service, presentation_id, "Time to see what the world thinks...\nIs social media friend or foe?")
    ...
    # Slide 10: TRANSITION - Hero Tool
    create_transition_slide(slides_service, presentation_id, "Here's your secret weapon...\nEvery detective needs the right tool")
    ...
    # Slide 13: TRANSITION - Practice
    create_transition_slide(slides_service, presentation_id, "Now it's your turn to crack the case...\nCan you find the tone in all 8 sections?")
    ...
    # Slide 17: TRANSITION - Reflection
    create_transition_slide(slides_service, presentation_id, "Let's reflect on what we discovered...\nWhat changed in your reading today?")
